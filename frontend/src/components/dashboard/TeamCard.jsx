    ...
    <div className="bg-[#1a1a1a] border border-neutral-800 rounded-2xl p-5 hover:border-neutral-700 transition-all duration-300 flex flex-col group">
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl bg-neutral-800 border border-neutral-700 flex items-center justify-center overflow-hidden shrink-0">
            {team.avatar && team.avatar !== "default-team.png" ? (
              <img src={team.avatar} alt={safeTeamName} className="w-full h-full object-cover" />
            ) : (
              <span className="text-xl font-bold text-neutral-500">{safeAvatarLetter}</span>
            )}
          </div>
        ...
        {/* Status Badge */}
        <span className={`text-[10px] uppercase tracking-wider font-bold px-2.5 py-1 rounded-full border shrink-0 ${
          isFull 
      ...
      <div className="mb-5">
        ...
        <div className="w-full bg-neutral-800 h-2 rounded-full overflow-hidden">
          <div 
            ...
            style={{ width: `${(((team.currentSize || team.teamSize || 1) / (team.maxMembers || 5)) * 100)}%` }}
          ></div>
        </div>
      </div>
      ...
      <div className="mt-auto pt-4 border-t border-neutral-800/80">
        <Link 
          to={`/dashboard/${isLeader ? 'manage' : 'team'}/${team.id || team.teamId || team._id}`}
